...
public class PostgresAclEventSubscriber implements ReactiveSubscriber {
    private final PostgresMailboxDAO postgresMailboxDAO;

    public PostgresAclEventSubscriber(PostgresMailboxDAO postgresMailboxDAO) {
        ...
        if (event instanceof ACLUpdated) {
            ...
            return Flux.fromStream(
                aclUpdated.getAclDiff()
                    .commands())
                .flatMap(command -> postgresMailboxDAO.updateACL((PostgresMailboxId) aclUpdated.mailboxId(), command))
                .then();
        }
        return Mono.empty();
    }
