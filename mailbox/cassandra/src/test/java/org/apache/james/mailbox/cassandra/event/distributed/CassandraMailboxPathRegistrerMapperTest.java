...
public class CassandraMailboxPathRegistrerMapperTest {
    ...
    private static final int CASSANDRA_TIME_OUT_IN_MS = 1;
    ...
    @Before
    public void setUp() {
        mapper = new CassandraMailboxPathRegisterMapper(cassandra.getConf(), cassandra.getTypesProvider());
        mapper.setCassandraTimeOutInS(CASSANDRA_TIME_OUT_IN_MS);
    }
    ...
    public void entriesShouldExpire() throws Exception {
        mapper.doRegister(MAILBOX_PATH, TOPIC);
        Thread.sleep(2000 * CASSANDRA_TIME_OUT_IN_MS);
        assertThat(mapper.getTopics(MAILBOX_PATH)).isEmpty();
    }
