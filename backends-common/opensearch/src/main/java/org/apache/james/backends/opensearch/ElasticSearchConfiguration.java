...
public class ElasticSearchConfiguration {
    ...
    public static class SSLConfiguration {
        ...
        private SSLConfiguration(SSLValidationStrategy strategy, HostNameVerifier hostNameVerifier, Optional<SSLTrustStore> trustStore) {
            ...
            Preconditions.checkArgument(strategy != SSLValidationStrategy.OVERRIDE || trustStore.isPresent(),  "%s strategy requires trustStore to be present", SSLValidationStrategy.OVERRIDE.name());
    ...
    private static SSLConfiguration sslTrustConfiguration(Configuration configuration) {
        SSLConfiguration.SSLValidationStrategy sslStrategy = Optional
            .ofNullable(configuration.getString(ELASTICSEARCH_HTTPS_SSL_VALIDATION_STRATEGY))
            .map(SSLConfiguration.SSLValidationStrategy::from)
            .orElse(SSLConfiguration.SSLValidationStrategy.DEFAULT);

        SSLConfiguration.HostNameVerifier hostNameVerifier = Optional
            .ofNullable(configuration.getString(ELASTICSEARCH_HTTPS_HOSTNAME_VERIFIER))
            .map(SSLConfiguration.HostNameVerifier::from)
            .orElse(SSLConfiguration.HostNameVerifier.DEFAULT);
