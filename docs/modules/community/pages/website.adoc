...
The source code of website https://james.apache.org[james.apache.org] is located in {path}.
...
Most of the useful content has been migrated but the new documentation is not yet published to the main site. It can be found on the https://james.staged.apache.org[staging version] of the website.
====
...
The resulting deployable content will be available in the `{path}/_site/` directory.
...
The maven site deploy phase uses and requires locally configuring credentials to enable ssh access. See
...
Enter the xref:contributing.adoc#_experimental_nix_support[james experimental development shell]
...
4. Wait for the changes to replicate to the Apache web server or setup 140.211.11.10:80 as
a proxy to review the changes (described here: http://www.apache.org/dev/project-site.html)
