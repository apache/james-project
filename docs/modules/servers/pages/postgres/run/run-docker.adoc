...
[subs=attributes+]
----
docker pull apache/james:postgres-{james-version}
----
...
[subs=attributes+]
----
$ docker run -d --network james --name=postgres --env 'POSTGRES_DB=james' --env 'POSTGRES_USER=james' --env 'POSTGRES_PASSWORD=secret1' postgres:{postgresql-version}
----
...
$ docker run --network james --hostname HOSTNAME -p "25:25" -p 80:80 -p "110:110" -p "143:143" -p "465:465" -p "587:587" -p "993:993" -p "127.0.0.1:8000:8000" --name james_run
    -v $PWD/keystore:/root/conf/keystore -t apache/james:postgres-{james-version} --generate-keystore
----
...
[subs=attributes+]
----
$ docker run -d --network james --name=rabbitmq rabbitmq:{rabbitmq-version}-management
----
...
[subs=attributes+]
----
$ docker run -d --network james --env 'REMOTE_MANAGEMENT_DISABLE=1' --env 'SCALITY_ACCESS_KEY_ID=accessKey1' --env 'SCALITY_SECRET_ACCESS_KEY=secretKey1' --name=s3 ghcr.io/scality/cloudserver:{cloudserver-version}
----

You need a running *OpenSearch* in docker which connects to *james* network. To achieve this run:

$ docker run -d --network james -p 9200:9200 --name=opensearch --env 'discovery.type=single-node' opensearchproject/opensearch:{opensearch-version}
...
[subs=attributes+]
----
$ docker run -d --network james --name tika apache/tika:{tika-version}
----
