...
Such metrics are exposed over HTTP and made available by using ``extensions.routes`` in https://github.com/apache/james-project/blob/master/docs/modules/servers/pages/distributed/configure/webadmin.adoc[webadmin.properties] file:
....
...
== Pre-load Prometheus datasource and dashboards to Grafana

You can download all the above dashboards and use https://grafana.com/tutorials/provision-dashboards-and-data-sources/[Grafana provision] to make the metrics datasource and dashboards available right after container creation.
...
Create the provisioning folder tree and copy all the dashboard json files to ``/provisioning/james/``
...
Sample config file for provisioned dashboard:
```
apiVersion: 1
...
Sample config file for provisioned datasource:
```
apiVersion: 1
...
datasources:
  - name: prometheus
    type: prometheus
    url: http://Prometheus_IP_Address:Port
    editable: true
...
Feel free to expore the dashboards you want.
