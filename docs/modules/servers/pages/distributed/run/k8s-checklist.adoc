...
* Check where your mail is filed, not only how it scores. Sending to one Gmail account tells you about that account; a service like link:https://email-spam-tester.com/[Email Spam Tester] reports inbox or spam folder at a dozen providers from a single test, and verifies SPF, DKIM and DMARC on the copy each of them received. On a fresh cluster this is what catches an IP with no reputation yet, or a signature broken by a relay in front of James.