...
* Docker ∕ OpenSearch {opensearch-version}, RabbitMQ Management {rabbitmq-version}, S3 compatible
ObjectStorage and Cassandra {cassandra-version}
* Maven 3
...
* RabbitMQ-Management {rabbitmq-version}
...
----
$ docker run -d -p 9042:9042 --name=cassandra cassandra:{cassandra-version}
...
$ docker run -d -p 5672:5672 -p 15672:15672 --name=rabbitmq rabbitmq:{rabbitmq-version}-management
$ docker run -d --env 'REMOTE_MANAGEMENT_DISABLE=1' --env 'SCALITY_ACCESS_KEY_ID=accessKey1' --env 'SCALITY_SECRET_ACCESS_KEY=secretKey1' --name=s3 ghcr.io/scality/cloudserver:{cloudserver-version}
----
