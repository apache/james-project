...
| Optional string. Configuration for ping mechanism in JMAP routes.
| Determine how often server will send a ping message to client through websocket connection.
| Supported units are: h (hour), m (minute), s (second), millisecond (ms). Default unit is second (s).
