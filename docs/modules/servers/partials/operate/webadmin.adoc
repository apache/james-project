...
....
curl -XDELETE "http://ip:port/blobs?action=initial-compaction&generation=1"
....
...
- *action*: (Compulsory) `initial-compaction`.
...
....
curl -XDELETE "http://ip:port/blobs?action=re-compaction&generation=1"
....

Query parameters:

- *action*: (Compulsory) `re-compaction`.
- *generation*: (Compulsory) The integer epoch generation to recompact.
- *family*: (Optional) Storage policy family identifier.
...
....
curl -XPOST http://ip:port/deletedMessages/users/user@domain.ext/messages[?force=true]
...
*Note*: The optional `force` query parameter (`?force=true` or `?action=restore&force=true`) bypasses
the user existence check. This is useful for restoring the vault of a deleted user or a virtual user.
