...
The <maxConcurrency> property controls the number of events processed in parallel. Defaults to 10.

The <executionTimeout> property (duration) controls the timeout for the execution of each listener. None if omitted.
...
| htmlBodyTemplate
| Optional. Mustache template for rendering an HTML body of the warning email. When specified, the email is sent as a
`multipart/alternative` message combining the plain text body (from `bodyTemplate`) and this HTML body. It is exposed the
