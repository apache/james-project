...
For the below conditions to be matched we assume RemoteDelivery is set up to categorise its failures with the
`IsRemoteDeliveryPermanentError` and `IsRemoteDeliveryTemporaryError` matchers, which is a requirement for detailed
