...
Backchannel logout is exposed as a WebAdmin route at `POST /add-revoked-token` and expects an `application/x-www-form-urlencoded`
body containing the `logout_token` parameter. The token payload must contain the `sid` claim to invalidate related cached access tokens.
