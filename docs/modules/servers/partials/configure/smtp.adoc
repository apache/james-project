...
We encourage setting this value to true on submission ports (465 + 587). MUST be set to false on port 25 or for any port with MX external mail receival role.
...
Just like for SMTP, the `errorMessages` element can be used to customize the human readable texts of some LMTP
