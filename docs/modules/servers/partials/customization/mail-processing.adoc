...
Finally, we will start a James server using that. We need to be using the *mailetcontainer.xml* configuration that
we had been writing, and to position the jar in the *conf/lib* folder, which the `bin/setenv.sh` script prepends to
