...
- *server/apps/postgres-app/pom.xml*
- *server/apps/postgres-app/docker-compose-distributed.yml*

[source]
```bash

    mvn clean install -Dmaven.javadoc.skip=true -DskipTests --pl :james-server-postgres-app --am

    docker compose -f server/apps/postgres-app/docker-compose-distributed.yml up -d
```
