...
[source]
----
$ docker run -d --network james -p 9042:9042 --name=cassandra cassandra:5.0.9
...
The `--family=<int>` and `--generation=<long>` arguments (also settable via the `RECOVERY_FAMILY` and
