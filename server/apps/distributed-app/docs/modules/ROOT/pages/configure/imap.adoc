...
| scheduler
| String. Possible value : immediate (default) and boundedElastic. Allows choosing the reactor scheduler. Use immediate
with async mailbox implementation (cassandra) and avoid it with blocking imlementations (like JPA).
|===
