...
public class PostgresThreadQueryViewDAO {
    ...
    public Flux<ThreadId> listLatestThreadIdsSortedByReceivedAt(PostgresMailboxId mailboxId, Limit limit) {
        return postgresExecutor.executeRows(dslContext -> Flux.from(dslContext
                .select(THREAD_ID, DSL.max(PostgresMessageDataDefinition.MessageToMailboxTable.INTERNAL_DATE).as("latest_date")) // Select thread_id and max internal_date
                .from(PostgresMessageDataDefinition.MessageToMailboxTable.TABLE_NAME)
                ...
                ThreadId.fromBaseMessageId(PostgresMessageId.Factory.of(record.get(PostgresThreadDataDefinition.PostgresThreadTable.THREAD_ID))));
    }
