...
public class FilterIntegrationTest {
    ...
    class ForwardTests {
        ...
        void forwardShouldWork() throws Exception {
            filteringManagementProbe.defineRulesForUser(BOB, asRule(Action.builder().setForward(Forward.to(CEDRIC.asMailAddress()).keepACopy())));
        ...
        void forwardShouldNotKeepACopyWhenKeepACopyIsFalse() throws Exception {
            filteringManagementProbe.defineRulesForUser(BOB, asRule(Action.builder().setForward(Forward.to(CEDRIC.asMailAddress()).withoutACopy())));
        ...
        void localCopyAloneShouldNotLeadToRecordingRRTError() throws Exception {
            filteringManagementProbe.defineRulesForUser(BOB, asRule(Action.builder().setForward(Forward.to().keepACopy())));
    ...
    class MoveToTests {
        ...
        void moveToShouldWork() throws Exception {
            ...
            mailboxProbe.createMailbox("#private", BOB.asString(), DefaultMailboxes.TRASH);

            filteringManagementProbe.defineRulesForUser(BOB, asRule(Action.builder()
                .setMoveTo(Optional.of(new Action.MoveTo(DefaultMailboxes.TRASH)))));
        ...
        @Test
        void moveToWithAppendInTheSameMailboxShouldNotDuplicateMails() throws Exception {
            MailboxProbeImpl mailboxProbe = jamesServer.getProbe(MailboxProbeImpl.class);
            MailboxId archiveMailboxId = mailboxProbe.createMailbox("#private", BOB.asString(), DefaultMailboxes.ARCHIVE);

            filteringManagementProbe.defineRulesForUser(BOB, asRule(Action.builder()
                .setAppendInMailboxes(Action.AppendInMailboxes.withMailboxIds(archiveMailboxId.serialize()))
                .setMoveTo(Optional.of(new Action.MoveTo(DefaultMailboxes.ARCHIVE)))));
