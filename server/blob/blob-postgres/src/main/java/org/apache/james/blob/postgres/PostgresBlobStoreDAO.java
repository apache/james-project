...
public class PostgresBlobStoreDAO implements BlobStoreDAO {
    ...
    public Publisher<Void> save(BucketName bucketName, BlobId blobId, InputStream inputStream) {
        ...
        }).flatMap(bytes -> Mono.from(save(bucketName, blobId, bytes)));
    }
    ...
    public Publisher<BucketName> listBuckets() {
        return postgresExecutor.executeRow(dsl -> Mono.from(dsl.selectDistinct(BUCKET_NAME)
                .from(TABLE_NAME)))
    ...
    public Publisher<BlobId> listBlobs(BucketName bucketName) {
        return postgresExecutor.executeRow(dsl -> Mono.from(dsl.select(BLOB_ID)
                .from(TABLE_NAME)
                .where(BUCKET_NAME.eq(bucketName.asString()))))
            .map(record -> blobIdFactory.from(record.get(BUCKET_NAME)));
    }
}
