...
public interface S3SSECConfiguration {
    ...
    String ENCRYPTION_S3_SSEC_ENABLE_PROPERTY = "encryption.s3.sse.c.enable";
    ...
    String ENCRYPTION_S3_SSEC_KEY_DERIVATION_ENABLED_PROPERTY = "encryption.s3.sse.c.key.derivation.enabled";

    static S3SSECConfiguration from(Configuration configuration) {
        boolean keyDerivationEnabled = configuration.getBoolean(ENCRYPTION_S3_SSEC_KEY_DERIVATION_ENABLED_PROPERTY, false);
        ...
        if (keyDerivationEnabled) {
            return new KeyDerivationEnabled(algorithm);
        }
    ...
    record KeyDerivationEnabled(String ssecAlgorithm) implements S3SSECConfiguration {
        public KeyDerivationEnabled {
            Preconditions.checkArgument(SUPPORTED_ALGORITHMS.contains(ssecAlgorithm), "Unsupported algorithm: " + ssecAlgorithm + ". The supported algorithms are: " + SUPPORTED_ALGORITHMS);
        }
    }
}
