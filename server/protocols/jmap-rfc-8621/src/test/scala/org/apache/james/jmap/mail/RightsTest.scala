...
class RightsTest extends AnyWordSpec with Matchers {
  ...
  "From ACL" should  {
    ...
    "filter out unknown rights" in {
      ...
      Rights.fromACL(MailboxACL.fromJava(acl)) must be(Rights.of(USER_ENTRYKEY, Seq(Right.Administer, Right.Expunge, Right.DeleteMessages, Right.DeleteMailbox, Right.CreateMailbox)))
    }
  }
  "To ACL" should  {
