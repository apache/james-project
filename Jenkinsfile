...
pipeline {
    ...
    stages {
        ...
        stage('Stable Tests') {
            steps {
                echo 'Running tests'
                sh 'mvn -B -e -fae test ${MVN_SHOW_TIMESTAMPS} -P ci-test -T1 ${MVN_LOCAL_REPO_OPT} -Dassembly.skipAssembly=true jacoco:report-aggregate@jacoco-report'
            }
            post {
                always {
        ...
        stage('Unstable Tests') {
            steps {
                ...
                catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
                    sh 'mvn -B -e -fae test -Punstable-tests ${MVN_SHOW_TIMESTAMPS} -P ci-test -T1 ${MVN_LOCAL_REPO_OPT} -Dassembly.skipAssembly=true'
                }
            }
            post {
                always {
